...
#define TAI_MINUS_UTC 37.0L // Time in seconds by which TAI is ahead of UTC.
...
#define GMEARTH_KM3_SEC2 398600.435507L // GM for the Earth: that is, the Universal
                                       // Gravitational Constant times the mass of Earth,
                                       // in units of km^3/sec^2.
...
#define BRACKETSCALE 10.0l
#define GOLDENSCALE 0.38197
#define POS_STEP 10.0l
...
struct LinkPurifyConfig {
  ...
  long maxrejnum = 50;         // Maximum number of points that can be rejected. Of rejfrac and maxrej, 
                               // whichever results in the smallest number of rejected points is operative.
  double max_astrom_rms = 1.0; // Maximum RMS astrometric residual, in arcsec.
  double ecc_penalty = 1.0;    // Factor by which to boost chi-square value to penalize high-eccentricty and interstellar orbit solutions.
  int minobsnights = 3;        // Minimum number of distinct observing nights for a valid linkage
...
public:
  ...
  hlclust(long clusternum, double posRMS, double velRMS, double totRMS, double astromRMS, int pairnum, double timespan, int uniquepoints, int obsnights, double metric, const string &rating, double reference_MJD, double heliohyp0, double heliohyp1, double heliohyp2, double posX, double posY, double posZ, double velX, double velY, double velZ, double orbit_a, double orbit_e, double orbit_incl, double orbit_MJD, double orbitX, double orbitY, double orbitZ, double orbitVX, double orbitVY, double orbitVZ, long orbit_eval_count) :clusternum(clusternum), posRMS(posRMS), velRMS(velRMS), totRMS(totRMS), astromRMS(astromRMS), pairnum(pairnum), timespan(timespan), uniquepoints(uniquepoints), obsnights(obsnights), metric(metric), reference_MJD(reference_MJD), heliohyp0(heliohyp0), heliohyp1(heliohyp1), heliohyp2(heliohyp2), posX(posX), posY(posY), posZ(posZ), velX(velX), velY(velY), velZ(velZ), orbit_a(orbit_a), orbit_e(orbit_e), orbit_incl(orbit_incl), orbit_MJD(orbit_MJD), orbitX(orbitX), orbitY(orbitY), orbitZ(orbitZ), orbitVX(orbitVX), orbitVY(orbitVY), orbitVZ(orbitVZ), orbit_eval_count(orbit_eval_count) { 
...
class shortclust{ // cluster (linkage) type used by low-memory version of heliolinc.
public:
  long clusternum;
  float posRMS;
  float totRMS;
  ...
  float velY;
  float velZ;
  shortclust(long clusternum, float posRMS, float totRMS, int pairnum, double metric, int hypindex, float posX, float posY, float posZ, float velX, float velY,float velZ) :clusternum(clusternum), posRMS(posRMS), totRMS(totRMS), pairnum(pairnum), metric(metric), hypindex(hypindex), posX(posX), posY(posY), posZ(posZ), velX(velX), velY(velY), velZ(velZ) { }
  shortclust() = default;
};
...
class uint_pair{ // Pair of unsigned integers
public:
  unsigned int i1;
  unsigned int i2;
...
class point3ix2{  // integer 3-D point plus 2 long integer indices
                  // The integer 3-D point is supposed to hold integerized
                  // versions of dynamical position state vectors (X, Y, Z).
                  // The purpose of the indices is to specify the pair of detections
                  // from which the state vector originated.
public:
  int x;
  int y;
  int z;
...
public:
  inline bool operator() (const point3ix2& p1, const point3ix2& p2) {
    return(p1.x < p2.x);
  }
};

class lower_point3ix2_y{ // Sort point3ix2's by y
public:
  inline bool operator() (const point3ix2& p1, const point3ix2& p2) {
    return(p1.y < p2.y);
  }
...
class lower_point3ix2_z{ // Sort point3ix2's by z
public:
  inline bool operator() (const point3ix2& p1, const point3ix2& p2) {
    return(p1.z < p2.z);
  }
...
class KD_point3ix2{  // 3-dimensional KD tree based on points of type point3ix2.
                     // These points carry two long integer indices not used by
                     // the KD tree, which itself uses two additional long integer
                     // indices to specify the left and right branches, and one
                     // regular integer to specify the dimension of the current
...
public:
  ...
  int dim;
  int flag;
  KD_point3ix2(point3ix2 point, long left, long right, int dim, int flag) :point(point), left(left), right(right), dim(dim), flag(flag) {}
};
...
double cluster_stats6i02(const vector <point6ix2> &cluster, double intconvscale, vector <double> &meanvals, vector <double> &rmsvals);
...
long medind_3ix2(const vector <point3ix2> &pointvec, int dim);
int split3ix2(const vector <point3ix2> &pointvec, int dim, long unsigned int splitpoint, vector <point3ix2> &left, vector <point3ix2> &right);
int kdtree_3i01(const vector <point3ix2> &invec, int dim, long unsigned int splitpoint, long unsigned int kdroot, vector <KD_point3ix2> &kdvec);
long point3ix2_dist2(const point3ix2 &p1, const point3ix2 &p2);
...
int statevec_to_celederiv(const vector <double> &baryvec, double &RA, double &Dec, vector <double> &RA_deriv, vector <double> &Dec_deriv, int verbose);
...
int link_dedup_lowmem(const vector <shortclust> &inclust, const vector  <uint_pair> &inclust2det, vector <shortclust> &outclust, vector  <uint_pair> &outclust2det);
int link_dedup_lowmem2(const vector <shortclust> &inclust, const vector  <uint_pair> &inclust2det, vector <shortclust> &outclust, vector  <uint_pair> &outclust2det);
...
int link_dedup2(const vector <hlclust> &inclust, const vector  <longpair> &inclust2det, vector <hlclust> &outclust, vector  <longpair> &outclust2det);
