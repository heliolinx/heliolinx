// November 10, 2023: modsplit_hlfile.cpp:
...
// linkages in the heliolinc output files -- hence, the different parts of
// files split by split_hlfile.cpp would have very different post-processing
// runtimes, while those generated by the current program (modsplit_hlfile.cpp)
// typically all require the same processing time to within a few percent.
...
{
  cerr << "Usage: modsplit_hlfile -sum summary_file -clust2det clust2detfile -splitnum number of parts -root root name for output cluster file\n";
}
...
int main(int argc, char *argv[])
{
  ...
  for(splitct=0; splitct<splitnum; splitct++) {
    ...
    outstream3 << "LP" << sumfile_part << " LP" << clust2detfile_part << "\n";
    ...
    for(subclustct=0;subclustct<subclustnum;subclustct++) {
      clustct = subclustct*splitnum + splitct;
      if(clustct<clustnum) {
	outstream1 << fixed << setprecision(3) << subclustct << "," << sumvecmain[clustct].posRMS << "," << sumvecmain[clustct].velRMS << "," << sumvecmain[clustct].totRMS << ",";
	...
	pointind = {};
	pointind = tracklet_lookup(clust2detmain, sumvecmain[clustct].clusternum);
	for(long i=0; i<long(pointind.size()); i++) {
	  outstream2 << subclustct << "," << pointind[i] << "\n";
	}
      }
    }
    outstream1.close();
