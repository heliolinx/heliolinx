// June 27, 2024:: heliovane.cpp:
// Like heliolinc, heliovane searches for asteroids using hypotheses
...
int main(int argc, char *argv[])
{
  ...
  while(i<argc) {
	...
	cerr << "Heliocentric ecliptic longitude, angular velocity, and acceleration\nfile keyword supplied with no corresponding argument\n";
	show_usage();
	return(1);
      }
  ...
  status=heliovane_alg_all(image_log, detvec, tracklets, trk2det, lambdahyp, earthpos, config, outclust, clust2det);
  if(status!=0) {
