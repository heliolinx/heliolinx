...
//
// May 20, 2024: Performs clustering in only 3 dimensions (X,Y,Z)
// instead of 6 (X, Y, Z, VX, VY, VZ) as, e.g., heliolinc_kd does.
//
...
int main(int argc, char *argv[])
{
  ...
  status=heliolinc_alg_R(image_log, detvec, tracklets, trk2det, radhyp, earthpos, config, outclust, clust2det);
  if(status!=0) {
    cerr << "ERROR: heliolinc_alg_R failed with status " << status << "\n";
    return(status);
  } 
